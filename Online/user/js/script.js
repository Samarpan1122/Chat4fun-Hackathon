...
(() => {
    ...
    const isValidID = (id) => {
        // Validate that the ID is a non-empty string and matches expected format
        const idPattern = /^[a-zA-Z0-9_-]+$/; // Example pattern, adjust as needed
        return typeof id === 'string' && idPattern.test(id);
    };

    const getURL = () => {
        if (urlResult === 'none' || !isValidID(urlResult)) { // Validate URL parameter
